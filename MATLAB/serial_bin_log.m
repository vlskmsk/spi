...
sin_mid = (max(sin_sig)+min(sin_sig))/2;
cos_mid = (max(cos_sig)+min(cos_sig))/2;
sin_amp = max(sin_sig)-min(sin_sig);
cos_amp = max(cos_sig)-min(cos_sig);

cos_correct = 1/cos_amp;
sin_correct = 1/sin_amp;
